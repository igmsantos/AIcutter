...
FANN_EXTERNAL void FANN_API fann_print_connections(struct fann *ann){
	char buffer[20000];
	fann_print_connections_str(ann,buffer);
	printf(buffer);
}

FANN_EXTERNAL void FANN_API fann_print_connections_str(struct fann *ann,char *output)
{
	...
	*output = '\0';
	sprintf(output,"Layer / Neuron \0");
	for(i = 0; i < num_neurons; i++)
	{
		sprintf((char*)(output + strlen(output)),"%d", i % 10);
	}
	...
	for(layer_it = ann->first_layer + 1; layer_it != ann->last_layer; layer_it++)
	{
		for(neuron_it = layer_it->first_neuron; neuron_it != layer_it->last_neuron; neuron_it++)
		{
			...
			sprintf((char*)(output + strlen(output)),"L %3d / N %4d %s\n\0", layer_it - ann->first_layer,
				   neuron_it - ann->first_layer->first_neuron, neurons);
		}
	}
	sprintf((char*)(output + strlen(output)),"\0");
	free(neurons);
}
